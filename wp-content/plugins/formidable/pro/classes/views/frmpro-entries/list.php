<div class="wrap">
    ...
    <h2><?php echo ($form) ? (FrmAppHelper::truncate($form->name, 25) .' ') : ''; _e('Entries', 'formidable'); ?>
...
<?php
    ...
    <?php foreach ($form_cols as $col){ ?>
        <td class="column-frm_<?php echo $col->field_key ?><?php if ($col == $form_cols[0]){ ?> post-title<?php } ?>">
            ...
            ?>
            <a class="row-title" href="?page=formidable-entries&amp;frm_action=<?php echo $entry_action ?>&amp;id=<?php echo $item->id; ?>" title="<?php echo esc_attr($entry_action .' '. $item->name); ?>">
            <?php }
...
<p><a href="<?php echo esc_url(FRM_SCRIPT_URL . "&controller=entries{$page_params}&frm_action=csv") ?>" class="button-secondary"><?php _e('Download CSV for', 'formidable'); echo ' '. $form->name; ?></a></p><br/><br/>
<?php } ?>
</form>
